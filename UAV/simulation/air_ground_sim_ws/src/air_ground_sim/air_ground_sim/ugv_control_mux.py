...
class UgvControlMux(Node):
    ...
    def on_timer(self) -> None:
        ...
        if not operator_fresh:
            self._publish_stop("operator_heartbeat_timeout")
            return
        if self.teleop_time > 0.0 and now - self.teleop_time <= self.command_timeout:
            self._publish(self.teleop_command, True, "operator_teleop", "operator_teleop")
        elif self.navigation_time > 0.0 and now - self.navigation_time <= self.command_timeout:
