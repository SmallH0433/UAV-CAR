...
参数：
  ...
  escape_path_points  (int,   6)    脱困路径点数
...
绕行策略（阿克曼不能原地自旋，全程保持 |v| > 0 或停车）：
  ...
  7. 原地往复检测：每次倒车脱困结束时记录当前雷达签名（前方各扇区净空），
     ...
     （escape_path_step × escape_path_points 折线，每步选一步落点净空
     最大的方向），沿路径走完后交回常规避障。
"""
...
class AvoidanceNode(Node):
    ...
    def _plan_escape_path(self):
        ...
        path = []
        for _ in range(self.escape_path_points):
